...
export default {
  ...
  methods: {
    goBack () {
      window.history.length > 1 ? this.$router.push('/dialogue&user=' + this.currUserData.user_Id) : this.$router.push('/')
    },
    getSelcet (choice) {
    ...
    setSelect (res) {
      ...
      if (res.code === 200) {
        if (res.ret) {
          this.$router.push('/PrivateInterFace/Object=' + this.objectChoice + '/selectAccountNumber=' + this.selectAccountNumber)
        } else {
