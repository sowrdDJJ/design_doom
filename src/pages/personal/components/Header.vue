  ...
  methods: {
        ...
        // on confirm
          if (this.ruleTelephone.test(this.personalData.user_Telephone)) {
            ...
            let personalDataNew = {
              ...
              'user_Id': this.personalData.user_Id,
            ...
            Axios.post('/data/postChangePirvateInformation', personalDataNew)
              .then(this.refreshuserHeadImg)
          } else {
            this.$toast('电话号码错误')
          }
        }).catch(() => {
    ...
    async refreshuserHeadImg (res) {
      ...
      if (res.ret && res.data) {
        ...
        this.$toast.clear()
        this.personalMore = !this.personalMore
        this.$set(data, 'user_Img', data.user_Img)
        this.$store.dispatch('obtionCurrUserData', data)
        Bus.$emit('updateUserHeadImg', data.user_Img)
      }
    },
    setPersonalInformation () {
      this.personalData = this.currUserData
...
.personal-header
  ...
  .ornaments
    ...
    border-bottom-left-radius: 28vh
    border-bottom-right-radius: 28vh
    box-shadow: 0vh 2vw 5vw $bgColorSecond;
