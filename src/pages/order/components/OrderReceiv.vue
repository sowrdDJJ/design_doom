            ...
            <div class="receivContent" v-for="item of orderReceivList" :key="item.id">
              ...
              <div class="receivContent-btns">
                  <ul>
                    ...
                    <router-link :to="`orderReceiv/passwordBtn/action=Receiv&number=` + item.id">
                      <li class="btns Receiv" @click="transtionOrderNavigation()">确认收货</li>
                    </router-link>
...
export default {
  ...
  methods: {
    getOrderReceiv () {
      ...
      }).then(this.inputOrderReceiv)
    },
    detailsBtn (id) {
      this.$router.push(`/commodity/id=` + id)
    },
    inputOrderReceiv (res) {
      res = res.data
      if (res.data && res.ret) {
        const data = res.data
        this.orderReceivList = data
      }
    },
    transtionOrderNavigation () {
